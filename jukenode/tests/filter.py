...
original = wave.open("sound1.wav", "r")
filtered = wave.open("sound8f.wav", "w")
...
original = [s / 256.0 for s in original]
...
for pos in range(0, len(original), FFT_SAMPLE):
    ...
    # scale back into WAV 8/16-bit (255/32767) and write
    time_domain = [ int(bound(sample) * 255) for sample in time_domain ]
    filtered.writeframes(
        struct.pack(
        '%dh' % len(time_domain),
        *time_domain
        )
